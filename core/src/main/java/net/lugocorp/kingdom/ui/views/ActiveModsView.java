...
class ActiveModsView implements View {
    ...
    ActiveModsView(StartMenuView.Params params) {
        ...
        for (GameMod mod : params.mods) {
            node.add(new SpacerNode()).add(new HeaderNode(params.av, mod.getName()))
                    .add(new TextNode(params.av, String.format("Key: %s", mod.getKey())))
                    .add(new TextNode(params.av, mod.getDescription()));
        }
